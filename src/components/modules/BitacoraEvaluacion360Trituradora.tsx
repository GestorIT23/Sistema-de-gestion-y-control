...
import type { Evaluacion360Trituradora, ItemEvaluacion360, AccionCorrectiva360 } from '../../types';
...
export default function BitacoraEvaluacion360Trituradora({ onBack, userEmail, onNavigateToEquipment }: Props) {
  ...
  const [registros, setRegistros] = useState<Evaluacion360Trituradora[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState({ text: '', type: '' });
  const [searchTerm, setSearchTerm] = useState('');
  const [recordToDelete, setRecordToDelete] = useState<Evaluacion360Trituradora | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);

  // Form general state
  const [folio, setFolio] = useState(() => 'EV360-TRIT-' + Math.floor(1000 + Math.random() * 9000));
  const [fecha, setFecha] = useState(new Date().toISOString().split('T')[0]);
  const [turno, setTurno] = useState<'Matutino' | 'Vespertino' | 'Nocturno'>('Matutino');
  const [equipoSeleccionado, setEquipoSeleccionado] = useState(EQUIPOS_TRITURADORA[0]);
  const [horometroActual, setHorometroActual] = useState(5280);
  const [operadorAsignado, setOperadorAsignado] = useState('Byron Estuardo Reyes (Operador Trituradora Shredder)');
  const [inspectorSgi, setInspectorSgi] = useState(userEmail || 'Auditor SGI Mantenimiento');
  const [observacionesGenerales, setObservacionesGenerales] = useState('Evaluación 360 integral de trituradora industrial shredder de doble eje para reducción de volumen.');

  // Equipment specific operational parameters
  const [amperajeMotorA, setAmperajeMotorA] = useState(62);
  const [velocidadRotacionRpm, setVelocidadRotacionRpm] = useState(24);
  const [tiempoRespuestaAutoReverseSeg, setTiempoRespuestaAutoReverseSeg] = useState(1.2);
  const [desgasteCuchillasMm, setDesgasteCuchillasMm] = useState(1.5);
  const [capacidadProcesamientoLbsHr, setCapacidadProcesamientoLbsHr] = useState(2500);

  // Dynamic evaluation items
  const [itemsSeguridad, setItemsSeguridad] = useState<ItemEvaluacion360[]>(DEFAULT_ITEMS_TRITURADORA.seguridad);
  const [itemsMecanico, setItemsMecanico] = useState<ItemEvaluacion360[]>(DEFAULT_ITEMS_TRITURADORA.mecanico);
  const [itemsHidraulicoCombustion, setItemsHidraulicoCombustion] = useState<ItemEvaluacion360[]>(DEFAULT_ITEMS_TRITURADORA.hidraulicoCombustion);
  const [itemsElectricoControl, setItemsElectricoControl] = useState<ItemEvaluacion360[]>(DEFAULT_ITEMS_TRITURADORA.electricoControl);
  const [itemsBioseguridadLimpieza, setItemsBioseguridadLimpieza] = useState<ItemEvaluacion360[]>(DEFAULT_ITEMS_TRITURADORA.bioseguridadLimpieza);
  const [itemsOperatividad, setItemsOperatividad] = useState<ItemEvaluacion360[]>(DEFAULT_ITEMS_TRITURADORA.operatividad);
  ...
  const [firmaOperador, setFirmaOperador] = useState('Byron Estuardo Reyes');
  ...
  const fetchRegistros = async () => {
    try {
      setLoading(true);
      const q = query(collection(db, 'bitacora_evaluacion_360_trituradora'), orderBy('fechaRegistro', 'desc'), limit(30));
      const querySnapshot = await getDocs(q);
      const docs: Evaluacion360Trituradora[] = [];
      querySnapshot.forEach((docSnap) => {
        const docData = sanitizeBiotrashObject(docSnap.data());
        docs.push({ id: docSnap.id, ...docData } as Evaluacion360Trituradora);
      });
      setRegistros(sortRecordsByDateDesc(docs, 'fecha'));
    } catch (e) {
      console.warn('Error fetching 360 trituradora records:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddAccion = () => {
    setAccionesCorrectivas(prev => [
      ...
      {
        id: 'AC-' + Date.now(),
        itemAfectado: 'Cuchillas / Ejes',
        descripcionDesviacion: 'Holgura de 1.8mm en disco 4 de eje secundario',
        accionPropuesta: 'Ajuste de espaciadores y reafilado programado para fin de mes',
        responsableEjecucion: 'Técnico Mecánico de Planta',
        plazoDias: 3,
        prioridad: 'Media',
        estado: 'Pendiente'
      }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const nuevoRegistro: Omit<Evaluacion360Trituradora, 'id'> = {
        folio,
        ...
        amperajeMotorA: Number(amperajeMotorA) || 0,
        velocidadRotacionRpm: Number(velocidadRotacionRpm) || 0,
        tiempoRespuestaAutoReverseSeg: Number(tiempoRespuestaAutoReverseSeg) || 0,
        desgasteCuchillasMm: Number(desgasteCuchillasMm) || 0,
        capacidadProcesamientoLbsHr: Number(capacidadProcesamientoLbsHr) || 0,
        ...
        cambioControl: [
          {
            ...
            fecha: '24/09/2026',
            seccion: 'Todas',
            cambio: 'Evaluación 360° digital de trituradora shredder de doble eje DSH',
            solicitante: 'Comité de Mantenimiento y Calidad'
          }
        ]
      };

      const sanitized = sanitizeBiotrashObject(nuevoRegistro);
      const docRef = await addDoc(collection(db, 'bitacora_evaluacion_360_trituradora'), sanitized);
      setRegistros(prev => [{ id: docRef.id, ...sanitized } as Evaluacion360Trituradora, ...prev]);

      setMsg({ text: '¡Auditoría 360° de Trituradora guardada exitosamente!', type: 'success' });
      setFolio('EV360-TRIT-' + Math.floor(1000 + Math.random() * 9000));
    } catch (err) {
      console.error('Error saving 360 trituradora:', err);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteDoc(doc(db, 'bitacora_evaluacion_360_trituradora', recordToDelete.id));
  ...
  const radarData = [
    ...
    { subject: 'Ejes/Cuchillas (20%)', valor: scoreMecanico, fullMark: 100 },
    { subject: 'Reductor (20%)', valor: scoreHidraulico, fullMark: 100 },
    { subject: 'Auto-Reverse (15%)', valor: scoreElectrico, fullMark: 100 },
    { subject: 'Nebulización (10%)', valor: scoreBioseguridad, fullMark: 100 },
    { subject: 'Granulometría (10%)', valor: scoreOperatividad, fullMark: 100 }
  ];
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 font-sans text-[#1A1C1E]">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-semibold overflow-x-auto">
          ...
          <button className="px-2.5 py-1 rounded-lg bg-emerald-700 text-white font-bold shadow-sm flex items-center gap-1.5">
            <Activity className="w-3.5 h-3.5" /> Trituradora
          </button>
        </div>
      </div>

      <FormHeader 
        titulo="EVALUACIÓN 360° DE TRITURADORA SHREDDER DE RESIDUOS (REDUCCIÓN DE VOLUMEN)"
        codigo="BIOTRASH 4.2. F-OPR-000-22"
        version="1.0"
      ...
      <div className="flex border-b border-slate-200 mb-6 bg-white rounded-t-xl px-4 pt-2 shadow-xs">
        ...
        >
          <Activity className="w-4 h-4 text-emerald-600" /> Formulario de Auditoría 360°
        </button>
        <button
        ...
        >
          <Award className="w-4 h-4 text-emerald-600" /> Radar de Confiabilidad 360°
        </button>
      </div>
      ...
      {activeTab === 'formulario' && (
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase text-slate-500">Amperaje / RPM</p>
                <p className="text-lg font-bold text-slate-800 mt-0.5">
                  {amperajeMotorA} A | {velocidadRotacionRpm} RPM
                </p>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase text-slate-500">Auto-Reverse / Capacidad</p>
                <p className="text-lg font-bold text-slate-800 mt-0.5">
                  {tiempoRespuestaAutoReverseSeg}s | {capacidadProcesamientoLbsHr} lb/h
                </p>
              </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide border-b border-slate-200 pb-2 mb-4 flex items-center gap-2">
              <Info className="w-4 h-4 text-emerald-600" /> I. Identificación de Trituradora y Parámetros
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Trituradora:</label>
                <select
                  value={equipoSeleccionado.id}
                  onChange={e => {
                    const match = EQUIPOS_TRITURADORA.find(eq => eq.id === e.target.value);
                    if (match) setEquipoSeleccionado(match);
                  }}
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg font-semibold"
                >
                  {EQUIPOS_TRITURADORA.map(eq => (
            ...
            {/* Shredder Specific Operational Variables */}
            <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 text-xs bg-emerald-50/40 p-3 rounded-xl border border-emerald-200">
              <div>
                <label className="block font-bold text-emerald-950 mb-1">Amperaje Motor (A):</label>
                <input 
                  type="number" 
                  value={amperajeMotorA} 
                  onChange={e => setAmperajeMotorA(Number(e.target.value))} 
                  className="w-full px-2.5 py-1.5 bg-white border border-emerald-300 rounded-lg font-bold"
                />
                <span className="text-[10px] text-slate-500">Nominal: 55 - 70 A</span>
              </div>

              <div>
                <label className="block font-bold text-emerald-950 mb-1">Velocidad Ejes (RPM):</label>
                <input 
                  type="number" 
                  value={velocidadRotacionRpm} 
                  onChange={e => setVelocidadRotacionRpm(Number(e.target.value))} 
                  className="w-full px-2.5 py-1.5 bg-white border border-emerald-300 rounded-lg font-bold"
                />
                <span className="text-[10px] text-slate-500">20 - 28 RPM (alto torque)</span>
              </div>

              <div>
                <label className="block font-bold text-emerald-950 mb-1">Respuesta Auto-Reverse (s):</label>
                <input 
                  type="number" 
                  step="0.1" 
                  value={tiempoRespuestaAutoReverseSeg} 
                  onChange={e => setTiempoRespuestaAutoReverseSeg(Number(e.target.value))} 
                  className="w-full px-2.5 py-1.5 bg-white border border-emerald-300 rounded-lg font-bold"
                />
                <span className="text-[10px] text-slate-500">&lt; 1.5 seg ante atasco</span>
              </div>

              <div>
                <label className="block font-bold text-emerald-950 mb-1">Desgaste Cuchillas (mm):</label>
                <input 
                  type="number" 
                  step="0.1" 
                  value={desgasteCuchillasMm} 
                  onChange={e => setDesgasteCuchillasMm(Number(e.target.value))} 
                  className="w-full px-2.5 py-1.5 bg-white border border-emerald-300 rounded-lg font-bold"
                />
                <span className="text-[10px] text-slate-500">Tolerancia &lt; 2.5 mm</span>
              </div>

              <div>
                <label className="block font-bold text-emerald-950 mb-1">Capacidad (lbs/hora):</label>
                <input 
                  type="number" 
                  value={capacidadProcesamientoLbsHr} 
                  onChange={e => setCapacidadProcesamientoLbsHr(Number(e.target.value))} 
                  className="w-full px-2.5 py-1.5 bg-white border border-emerald-300 rounded-lg font-bold"
                />
                <span className="text-[10px] text-slate-500">2,000 - 3,000 lb/h</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3 mb-4">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide flex items-center gap-2">
                <CheckSquare className="w-4 h-4 text-emerald-600" /> II. Matriz de Auditoría 360° de Trituradora
              </h3>
              ...
              <div className="flex flex-wrap gap-1 text-xs font-semibold">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg transition ${
                    categoryTab === 'seguridad' ? 'bg-emerald-700 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  Seguridad ({scoreSeguridad}%)
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg transition ${
                    categoryTab === 'mecanico' ? 'bg-emerald-700 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  Ejes & Cuchillas ({scoreMecanico}%)
                </button>
                <button
                  type="button"
                  onClick={() => setCategoryTab('hidraulicoCombustion')}
                  className={`px-3 py-1.5 rounded-lg transition ${
                    categoryTab === 'hidraulicoCombustion' ? 'bg-emerald-700 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  Transmisión ({scoreHidraulico}%)
                </button>
                <button
                  type="button"
                  onClick={() => setCategoryTab('electricoControl')}
                  className={`px-3 py-1.5 rounded-lg transition ${
                    categoryTab === 'electricoControl' ? 'bg-emerald-700 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  Auto-Reverse ({scoreElectrico}%)
                </button>
                <button
                  type="button"
                  onClick={() => setCategoryTab('bioseguridadLimpieza')}
                  className={`px-3 py-1.5 rounded-lg transition ${
                    categoryTab === 'bioseguridadLimpieza' ? 'bg-emerald-700 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  Nebulización ({scoreBioseguridad}%)
                </button>
                <button
                  type="button"
                  onClick={() => setCategoryTab('operatividad')}
                  className={`px-3 py-1.5 rounded-lg transition ${
                    categoryTab === 'operatividad' ? 'bg-emerald-700 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  Granulometría ({scoreOperatividad}%)
                </button>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide flex items-center gap-2">
                <ShieldAlert className="w-4 h-4 text-emerald-600" /> III. Plan de Acciones Correctivas
              </h3>
              <button
                type="button"
                onClick={handleAddAccion}
                className="px-3 py-1.5 bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200 rounded-lg font-bold text-xs flex items-center gap-1 transition"
              >
            ...
            {accionesCorrectivas.length === 0 ? (
              <p className="text-xs text-slate-500 italic p-3 bg-slate-50 rounded-lg border border-slate-200">
                Trituradora operando dentro de los límites de desgaste de cuchillas y par motor. Sin desviaciones críticas.
              </p>
            ) : (
              <div className="space-y-3">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide border-b border-slate-200 pb-2 mb-4 flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-emerald-600" /> IV. Observaciones y Firmas SGI
            </h3>
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 pt-2">
            <button
              type="submit"
              disabled={saving}
              className="px-6 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl shadow-sm flex items-center gap-2 text-xs sm:text-sm transition disabled:opacity-50"
            >
      ...
      {activeTab === 'historico' && (
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => generateAndDownloadExcel('evaluacion_360_trituradora', { results: registros })}
              ...
              {isAuthorizedToDelete(userEmail) && (
                <GestorItDeleteModuleRecords 
                  collectionName="bitacora_evaluacion_360_trituradora"
                  moduleName="Evaluación 360 Trituradora"
                  onDeleted={() => {
                    setRegistros([]);
                    setMsg({ text: 'Registros de evaluación 360 de trituradora eliminados.', type: 'success' });
                  }}
                  userEmail={userEmail}
                />
              )}
          ...
          {loading ? (
            <div className="py-12 text-center text-slate-400 text-xs flex flex-col items-center gap-2">
              <RefreshCw className="w-5 h-5 animate-spin text-emerald-600" /> Cargando auditorías de trituradora...
            </div>
          ) : filteredRegistros.length === 0 ? (
            <div className="py-12 text-center text-slate-500 text-xs italic bg-slate-50 rounded-xl border border-slate-200">
              No se encontraron registros de auditoría para trituradoras.
            </div>
          ) : (
                    ...
                    <tr key={reg.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="p-2.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            onClick={() => generateAndDownloadPDF('evaluacion_360_trituradora', reg)}
                          ...
                          <button
                            onClick={() => generateAndDownloadExcel('evaluacion_360_trituradora', reg)}
      ...
      {activeTab === 'analitica' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-7 bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide border-b border-slate-200 pb-2 mb-4 flex items-center gap-2">
              <Award className="w-4 h-4 text-emerald-600" /> Evaluación Radial 360° Trituradora Shredder
            </h3>
            <div className="h-80 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarData}>
                  ...
                  <Radar name="Conformidad (%)" dataKey="valor" stroke="#059669" fill="#34D399" fillOpacity={0.45} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="font-semibold text-slate-700">Seguridad & Paradas Emergencia</span>
                  <span className="font-bold text-emerald-700">{scoreSeguridad}%</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="font-semibold text-slate-700">Ejes de Corte & Cuchillas</span>
                  <span className="font-bold text-emerald-700">{scoreMecanico}%</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="font-semibold text-slate-700">Caja Reductora & Transmisión</span>
                  <span className="font-bold text-emerald-700">{scoreHidraulico}%</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="font-semibold text-slate-700">Control Auto-Reverse & Amperaje</span>
                  <span className="font-bold text-emerald-700">{scoreElectrico}%</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="font-semibold text-slate-700">Nebulización & Supresión Polvo</span>
                  <span className="font-bold text-emerald-700">{scoreBioseguridad}%</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="font-semibold text-slate-700">Rendimiento & Granulometría</span>
                  <span className="font-bold text-emerald-700">{scoreOperatividad}%</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {recordToDelete && (
        <DeleteSingleRecordModal 
          ...
          title="¿Eliminar evaluación 360° de trituradora?"
      ...
      <div className="mt-8">
        <FormFooter 
          elaboroCargo="Comité SGI / Mantenimiento"
          revisoCargo="Jefatura de Planta"
